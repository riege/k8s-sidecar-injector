...
var (
	...
	testGoodConfigs = map[string]testhelper.ConfigExpectation{
		...
		"envfrom1": testhelper.ConfigExpectation{
			Name:               "envfrom1",
			Version:            "latest",
...
func TestGoodConfigs(t *testing.T) {
	for _, testConfig := range testGoodConfigs {
		...
		if len(c.EnvFrom) != testConfig.EnvFromCount {
			t.Fatalf("expected %d EnvFroms loaded from %s but got %d", testConfig.EnvFromCount, testConfig.Path, len(c.EnvFrom))
		}
