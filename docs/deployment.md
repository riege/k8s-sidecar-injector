...
Verify its up and running; note the `injector.riege.com/status: injected` label, indicating the pod had its sidecar added successfully, as well as the added environment variables, and additional `sidecar-nginx` container!
...
Namespace:    default
...
Annotations:  injector.riege.com/status: injected
Status:       Running
IP:           10.246.248.115
Containers:
  debian-debug:
